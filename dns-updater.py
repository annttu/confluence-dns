...
class DNSUpdater(object):
    ...
    def generate_zonefile(self, name, records, zone):
        ...
        out("$TTL 60")
        out("@ IN SOA %s %s %d 3H 2H 2W 6H" % (zone['master'], zone['hostmaster'], time.time()))
        out("")
        for dns_server in zone['servers']:
            out("@ IN NS %s" % dns_server)
        out("")
            ...
            if 'ZONE' in lines[0]:
                for line in lines:
                    if not 'ZONE' in line or line['ZONE'] is None:
                        continue
                    zones[line['ZONE'].split("#", 1)[1].strip()] = {'hostmaster': line['hostmaster'].strip(),
                                                                    'master': line['master'].strip(),
                                                                    'servers': [x.strip() for x in fix_confluence_spaces(line['servers']).split()]}
                continue
            if 'Name' not in lines[0] or not lines[0]['Name'].startswith('# '):
                continue
            for line in lines:
                if not 'Name' in line or line['Name'] is None:
                    continue
                elif line['Name'].startswith("# "):
                    network = line['Name'].split("#",1)[1].strip()
    ...
    def generate_forward_records(self, table, zones):
        ...
        for network, items in table.items():
            ...
            for item in items:
                if network_part:
                    name = "%s.%s" % (item['Name'], network_part)
                else:
                    name = item['Name']
